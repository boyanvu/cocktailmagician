...
{
    [TestClass]
    public class Ingredient_IsUnlisted_Should
    {
        [TestMethod]
        ...
        [TestMethod]
        public async Task Throw_False_If_CI_IsNull()
        {
            var options = Utils.GetOptions(nameof(Throw_False_If_CI_IsNull));
            ...
            using (var assertContext = new CMContext(options))
            {
                ...
                var result = await sut.IngredientIsUnlisted(cocktailId, ingredientId);
