...
{
    [TestClass]
    public class Remove_CReview_Like_Should
    {
        [TestMethod]
        public async Task Remove_Cocktail_Review_Like()
        {
            var options = Utils.GetOptions(nameof(Remove_Cocktail_Review_Like));
            ...
            using (var arrangeContext = new CMContext(options))
            {
                ...
                await sut.RemoveCocktailReviewLike(cReview.Id, user.UserName);
            }
            ...
            using (var assertContext = new CMContext(options))
            {
                ...
                Assert.AreEqual(false, assertContext.CocktailReviewLikes.First().IsLiked);
            }
        }

        [TestMethod]
        public async Task Throw_When_RCRL_CR_IsNull()
        {
            var options = Utils.GetOptions(nameof(Throw_When_RCRL_CR_IsNull));
            ...
            using (var assertContext = new CMContext(options))
            {
                ...
                await Assert.ThrowsExceptionAsync<ArgumentNullException>
                    (async () => await sut.RemoveCocktailReviewLike(crId, user.UserName));
            }
        }

        [TestMethod]
        public async Task Throw_When_RCRL_User_IsNull()
        {
            var options = Utils.GetOptions(nameof(Throw_When_RCRL_User_IsNull));
            ...
            using (var assertContext = new CMContext(options))
            {
                var sut = new CocktailReviewLikeService(assertContext);
                await Assert.ThrowsExceptionAsync<ArgumentNullException>
                    (async () => await sut.RemoveCocktailReviewLike(cReview.Id, "Pesho"));
            }
        }

        [TestMethod]
        public async Task Throw_When_RCRL_CRL_IsNull()
        {
            var options = Utils.GetOptions(nameof(Throw_When_RCRL_CRL_IsNull));
            ...
            using (var assertContext = new CMContext(options))
            {
                var sut = new CocktailReviewLikeService(assertContext);
                await Assert.ThrowsExceptionAsync<ArgumentNullException>
                    (async () => await sut.RemoveCocktailReviewLike(cReview.Id, user.UserName));
            }
        }
    }
}
